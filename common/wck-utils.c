...
static gboolean window_is_in_monitor(XfwWindow* win, XfwMonitor *m) {
    ...
    while (monitors && monitors->data)
    {
        if (monitors->data == m) return TRUE;
        monitors = monitors->next;
    }
...
static XfwWorkspace *get_active_workspace(XfwScreen *screen, XfwMonitor *monitor, gboolean only_current_display)
{
	...
	while(groups && groups->data) {
		...
		if (g_list_find(monitors, monitor) || ! only_current_display)
			return xfw_workspace_group_get_active_workspace(groups->data);

		groups = groups->next;
	}

	GList *workspaces = xfw_workspace_manager_list_workspaces(mgr);
...
void init_wnck (WckUtils *win, gboolean only_maximized, gboolean only_current_display, gpointer data)
{
    ...
    win->activeworkspace = get_active_workspace(win->activescreen, win->monitor, only_current_display);

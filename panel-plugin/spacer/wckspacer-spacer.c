...
void on_control_window_changed (XfwWindow *controlwindow, XfwWindow *previous, gpointer data)
{
    ...
    if (!controlwindow
        || (window_is_desktop (controlwindow)
            && !wsp->prefs->show_on_desktop))
    {
        if (gtk_widget_get_visible(GTK_WIDGET(wsp->plugin)))
            gtk_widget_hide(GTK_WIDGET(wsp->plugin));
    }
    else
    {
        if (!gtk_widget_get_visible(GTK_WIDGET(wsp->plugin)))
            gtk_widget_show_all(GTK_WIDGET(wsp->plugin));
    }
...
{
    if (event->button == 1) {
        if(window_is_desktop (wsp->win->controlwindow) && !wsp->prefs->show_on_desktop)
            return FALSE;

        xfw_window_activate (wsp->win->controlwindow, NULL, GDK_CURRENT_TIME, NULL);
        return TRUE;
    }

    if (event->button == 3 && (
        (event->state & GDK_CONTROL_MASK) == GDK_CONTROL_MASK
        || window_is_desktop (wsp->win->controlwindow)
    )) {
